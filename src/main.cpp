...
static CBigNum bnMaxProofOfWorkLimit(~uint256(0) >> 27);
static CBigNum bn100WMaxProofOfWorkLimit(~uint256(0) >> 34);
...
const string strMessageMagic = "Eatcoin Signed Message:\n";
...
{
	if (pindexBest != NULL) {
		if (pindexBest->nHeight == 1 || pindexBest->nHeight == 2) {
			CBigNum T = MAX_MONEY / 2;
			return T.getuint64();
...
#endif
{
	...
	if ((fProofOfStake && pindexLast->nHeight < 1000000) || pindexLast->nHeight < 500000) {
		if (bnNew < bnMaxProofOfWorkLimit)
			bnNew = bnMaxProofOfWorkLimit;
	} else {
		if (pindexLast->nHeight < 1000000) {
			if (bnNew < bn100WMaxProofOfWorkLimit)
				bnNew = bn100WMaxProofOfWorkLimit;
		}
	}
...
bool LoadBlockIndex()
{
    ...
    if (fTestNet)
    {
        ...
        nStakeMinAge = 60 ; // test net min age is 1 minutes
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Eatcoin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
		// date -s "2017-09-12 02:27:05"
        const char* pszTimestamp = "EAT, 2018.01.31 GuangZhou EAlliance Co., Ltd";
        CTransaction txNew;
        txNew.nTime = 1517407470;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1517407542;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2160959;

        if (fTestNet)
        {
            block.nTime    = 1517407548;
            block.nNonce   = 0;
        }
		...
		assert(block.hashMerkleRoot == uint256("38543bbe7980aaa4f5c5c94c76d624e8954768b197fb38f76382e381b298fc0d"));
